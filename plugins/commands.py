...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.reply(script.START_TXT.format(mention=message.from_user.mention if message.from_user else message.chat.title, username=temp.U_NAME, bot_name=temp.B_NAME), reply_markup=reply_markup, disable_web_page_preview=True)
    ...
    if len(message.command) != 2:
        ...
        await message.reply_photo(
            ...
            caption=script.START_TXT.format(mention=message.from_user.mention, username=temp.U_NAME, bot_name=temp.B_NAME),
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('💎 ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ', callback_data='subscription')
            ],[
                InlineKeyboardButton('📢 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url=CHNL_LNK)
            ]]
        else:
