        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ...
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b>🔍 ꜱᴇᴀʀᴄʜɪɴɢ ꜰᴏʀ <code>{message.text}</code>...</b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b>🔍 ꜱᴇᴀʀᴄʜɪɴɢ ꜰᴏʀ <code>{content}</code>...</b>", reply_to_message_id=message.id)
    ...
    if query.data.startswith("file"):
     ...
     #   if not search:
      ...
      #      return
        ...
        await query.message.edit_text(
            text=script.START_TXT.format(mention=query.from_user.mention, username=temp.U_NAME, bot_name=temp.B_NAME),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        await query.message.edit_text(
            text=script.ALL_FILTERS.format(mention=query.from_user.mention),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        buttons = [[
             ...
             InlineKeyboardButton('🎨 ᴄᴜꜱᴛᴏᴍɪᴢᴇ', callback_data='r_txt')
         ], [
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    return await reply_msg.edit_text(f"**<blockquote>⚠️ ɴᴏ ꜰɪʟᴇꜱ ꜰᴏᴜɴᴅ ꜰᴏʀ ʏᴏᴜʀ ǫᴜᴇʀʏ: <code>{name}</code>\n\nᴍᴀᴋᴇ ꜱᴜʀᴇ ᴛʜᴇ ꜱᴘᴇʟʟɪɴɢ ɪꜱ ᴄᴏʀʀᴇᴄᴛ!</blockquote>**")
        else:
            return
    else:
    ...
    if imdb:
        ...
        cap = f"<b><blockquote>🔍 ꜱᴇᴀʀᴄʜ ʀᴇꜱᴜʟᴛꜱ ꜰᴏʀ: {search}</blockquote>\n\n✨ ʀᴇǫᴜᴇꜱᴛᴇᴅ ʙʏ: {message.from_user.mention}\n⏰ ꜱᴇᴀʀᴄʜ ᴛɪᴍᴇ: <code>{remaining_seconds}ꜱ</code>\n\n🍿 ʜᴀᴠᴇ ꜰᴜɴ ᴡᴀᴛᴄʜɪɴɢ!</b>"
        if not settings["button"]:
            cap += "\n\n<b><u>📂 ᴀᴠᴀɪʟᴀʙʟᴇ ꜰɪʟᴇꜱ 👇</u></b>\n"
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n</a></b>"
...
from plugins.Extra.engine import ai as ai_engine
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        await reply_msg.edit_text("<b><blockquote>🤖 ᴀɪ ɪꜱ ᴀɴᴀʟʏᴢɪɴɢ ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛ...</blockquote></b>")

        try:
            ai_query = f"The user searched for movie '{mv_rqst}' but no files were found. Suggest the most likely correct movie title. Only return the title."
            suggested_title = await ai_engine(ai_query)
            if suggested_title and suggested_title.lower() != mv_rqst.lower():
                return await auto_filter(client, suggested_title, msg, reply_msg, vj_search_new)
        except Exception as e:
